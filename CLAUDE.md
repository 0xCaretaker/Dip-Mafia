...
5. **Telegram + Discord delivery** (`bot.py:send_bulk_telegram_message`, `send_discord_message`). The same `final_message` payload goes to both channels. Telegram uses MarkdownV2 to multiple chat IDs from `TELEGRAM_CHAT_IDS`. Discord is opt-in via `DISCORD_WEBHOOK_URL`; `_md_v2_to_discord` strips `\\(`/`\\.` escapes and promotes `*X*` (bold in TG) to `**X**` (bold in Discord — single-star is italic there), then splits on blank lines to stay under the 2000-char cap. Discord posts prepend `@here` with `allowed_mentions.parse=["everyone"]` so the ping actually fires. Message sections (built in `build_message`): the **Verdict** (Impulse MACD restricted to the Bollinger-filtered `{Buy, Watch}`+below-mid universe via `passes_bollinger_gate`), then a positional **Near Value** radar. The ungated full-universe **Early Signal** (Standard MACD) and **Strong Signal** (Impulse MACD) lists were **removed from the notification (2026-07-05)** — they are console-only now, printed in full to the GitHub Action log (as is a dedicated `BOLLINGER FILTER PASS` list: the exact `passes_bollinger_gate` set the Verdict draws from, with each stock's BB action + mid distance). The top-of-message `Sentiment` line still summarizes the Bollinger-filtered Hold/Wait mood. Near Value lists Top-50 names (`six7_set`) trading below the 200-SMA midline (`mid_dist_pct < 0`, from `bollinger_signals.calculate_bb_mid_distance_pct`), sorted cheapest-first, each line = band-position icon + symbol + signed % from mid + a `⚡` when the Impulse MACD just crossed up (fresh `Buy`). It is deliberately **NOT** routed through the Bollinger gate — it surfaces cheap Top-50 names (e.g. below-mid with no lower-band touch, the case the Verdict correctly excludes) purely for awareness, and carries a short `💰 cash in hand? buy these below-mid names now` prompt directly under its heading (an actionable nudge to deploy spare cash). When the Verdict has no Buy/Sell entries it prints a tight `🚫 no buys today` line under the title (no blank gap) before the Wait/Hold stats. It renders even when the Verdict is empty, but is suppressed (with the rest of the message) when nothing at all qualifies. Header includes NIFTY 50 and NIFTY Midcap 100 day move + % from ATH (`get_index_moves`). Each MACD stock line carries a class tag (`⭐` Top 50 / `💼` your holding) so six7 names and held-but-not-Top-50 names are distinguishable; the tag is computed from `six7_set` (overlap → six7 wins).
