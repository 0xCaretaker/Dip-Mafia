...
2. **MACD crossovers as the long signal** (`macd_signals.py`). Two indicators run in parallel on the same data:
   ...
   Both feed `_trend_to_action`, which walks the crossover series and emits the *latest* state: `Buy`/`Sell` on the crossover bar, then `Hold` (after Buy) or `Wait for Buy` (after Sell) until the next cross. Only `Buy`/`Sell` rows render as priced stock lines in Telegram. `Hold` is a bare count; `Wait for Buy` is a count **plus the names** (sorted, each with its `⭐`/`💼` class tag, no price - they are not actionable yet, but they are the list worth watching for tomorrow).
