...
Segmented control (nav + horizon), stat tile, **lead tile** (the one headline metric per section: raised `--surface-2`, spans 2 grid cols, figure at 2.7rem/700 vs the 1.6rem/600 of regular tiles - this size+weight jump is the hierarchy spine, deliberately breaking the uniform grid), chart panel, data table (sticky head, hover row, sortable), pill/tag, tooltip+crosshair, donut legend, comparison table (best-per-row highlight). Figures lead: mono, tabular, weight 600-700, tight tracking; labels stay quiet (mono, uppercase, `--ink-mute`). The Overview hero number is the single largest element (Fraunces, clamp to 5.4rem, well under 6) with `₹` and `Cr`/`L` rendered as muted superscript affixes so the digits dominate. Data tables carry **inline sparklines** (a Trend column) and are **row-drillable**: a holdings row opens a modal with that stock's price chart + buy markers + trade ledger; a Screens leaderboard row selects and scrolls to that list's inspect panel. The Iterations grid (metrics x watchlists) carries a **Trend row** of per-watchlist equity sparklines instead of a column. Rows that have no series (benchmark rows) show `–` and stay inert. The modal dismisses on section change. Stat tiles can be **navigable** (`tile(..., link)`): a summary tile whose fuller detail lives in another section becomes a clickable affordance (cursor, hover lift, a `↗` glyph, role=link + keyboard) that jumps there - e.g. Overview's comparison tiles link into Backtest. Touch devices get enlarged hit areas (~44px) for the segmented controls and nav via `@media(pointer:coarse)`. Each interactive element: default / hover / focus-visible / active / disabled. Empty + loading states. Motion 150-250ms ease-out; horizon/section changes cross-fade; charts draw-in once (reduced-motion: instant). No page-load choreography.

## Charts (hand-built SVG)

One small module: `lineChart` (multi-series, optional fill, hover crosshair+tooltip with an optional `tipExtra` row e.g. profit = value-invested, draw-in), `barChart` (vertical/horizontal, grouped), `donut`. Tabular mono axes, `--line` grid, series colors above. **Every chart is interactive** (the data is the product): lines get the crosshair+tooltip; bars dim the rest and show a label+series+value tooltip on hover; the donut pops the hovered segment, dims the others, and reads value + share in its center. Charts are **width-responsive**: each SVG's viewBox tracks its container's pixel width (not a fixed 760) and the active section re-renders on resize, so axis text and strokes stay crisp at every breakpoint instead of scaling down to mush on mobile.

Two table-scale chart primitives: `sparkline(values)` - a bare ~90x26 trend line (no axes), coloured green/red by net move, embedded in a table cell; and `priceMini(host, spark, trades, avg, cmp)` - the holdings drilldown chart (price line + area, amber buy-point markers placed by date fraction, dashed avg-cost reference). Per-row `spark` data (a compact ~40-point windowed close series with `d0`/`d1` endpoints) is emitted by `horizon_compare.py`.
