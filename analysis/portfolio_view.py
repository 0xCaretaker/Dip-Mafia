...
Reads (from the newest run subfolder under backtest_output/, via run_paths):
  - <current run>/trades.csv (holdings for portfolio tab)
  - <current run>/dashboard_data.json (backtest results, generated by backtest.py)

Run: python3 analysis/portfolio_view.py   (from the repo root)
Output: dashboard.html (+ published copy in docs/strat.html)
"""
...
# Read the newest run subfolder (the "current" run) under backtest_output/.
...
def load_iterations(current_data):
    """Current run first (newest subfolder), then every other run under
    backtest_output/.

    Each run is a self-contained snapshot (its own meta.json + stocks.txt), so
    new iterations appear automatically once their folder exists - no code
    change needed. meta.json enriches the label.
    """
    iters = []
...
function renderIterations() {{
  ...
  let html = `<div class="assumptions">
    ...
    Each run is a dated snapshot kept in its own subfolder under
    <code>backtest_output/</code> - new iterations appear here automatically.
...
def main():
    ...
    horizons = None
    try:
        with open(os.path.join(CURRENT_RUN, "horizons.json")) as f:
